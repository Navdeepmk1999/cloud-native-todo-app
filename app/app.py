...
client = MongoClient(f"mongodb://{MONGO_HOST}/test")
...
@app.route("/hello")
def index():
	return f'<h1> Hello World <br/> {MONGO_HOST} </h1>'

@app.route('/health')
def health():
    # Randomly return 200 or 500 status to simulate a liveness check failure
    if random.choice([True, False]):  # 50% chance to succeed or fail
        return "OK", 200
    else:
        return "Error", 500
...
@app.route('/readiness')
def readiness():
    # Randomly return 200 or 500 status to simulate a readiness check failure
    if random.choice([True, False]):  # 50% chance to succeed or fail
        return "Ready", 200
    else:
